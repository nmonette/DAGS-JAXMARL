...
class EnemySMAX(MultiAgentEnv):
    ...
    def step_env(
        ...
        enemy_actions, enemy_policy_state = self.get_enemy_actions(
            action_key, state.enemy_policy_state, enemy_obs, self.get_enemy_avail_actions(state)
        )
    ...
    @partial(jax.jit, static_argnums=(0,))
    def get_enemy_avail_actions(self, state: State):
        avail_actions = self._env.get_avail_actions(state.state)
        return {agent: avail_actions[agent] for agent in self.enemy_agents}
...
class Env(LearnedPolicyEnemySMAX):
    def get_enemy_actions(self, key, policy_state, enemy_obs, avail_actions):

        ac_in = (
            enemy_obs[None],
            jnp.zeros((1,1)),
            batchify(avail_actions, self.enemy_agents, len(self.enemy_agents)),
        )
        hstate, pi, _ = self.policy.apply(policy_state["params"], policy_state["hstate"], ac_in)
        enemy_actions = pi.sample(seed=key).squeeze()
        enemy_actions = {
            agent: enemy_actions[self._env.agent_ids[agent] - self.num_agents]
            for agent in self.enemy_agents
        }
        ...
        enemy_actions = {k: v.squeeze() for k, v in enemy_actions.items()}
        return enemy_actions, {"hstate": hstate, "params": policy_state["params"]}